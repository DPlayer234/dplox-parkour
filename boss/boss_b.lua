...
local boss = {
	anim = anim.newAnimHead(love.graphics.newImage("assets/boss/sandypress.png"),46,50),
	x = 135,
	y = 209,
	var = 0,
	hp = 8,
	d = {x=135,y=209,var=0,hp=8,status="idle"},
	status = "idle",
	update = function()
		if boss.status == "idle" then
			if player.x > boss.x+26 then
				boss.x = boss.x + (9-boss.hp)/2
		...
		elseif boss.status == "attack" then
			if boss.var < boss.hp*2.5 + 2.5 then
				boss.var = boss.var + 1
	...
	draw = function()
		...
		if boss.status == "idle" then
			local n = math.ceil(boss.var/24)
			bar[2] = "  "..string.rep("!  ",n)
			bar[4] = string.rep("!  ",5-n)
